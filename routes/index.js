...
router.get('/add-to-wishlist/:id', middleware.isLoggedIn, async (req, res) => {
  ...
  try {
    let user_wish;
    //if user, check wish in database
    if (req.user) {
      user_wish = await Wish.findOne({user: req.user._id})
    }
    let wish;
    if(!user_wish){
      wish = new Wish({});
    ...
    const wishIndex = wish.items.findIndex((p) => productId == p);
    if (wishIndex > -1) {
      wish.items.splice(wishIndex, 1);
    } else {
      wish.items.push(productId);
    }
    wish.totalQty = wish.items.length;
    if (req.user) {
      wish.user = req.user._id.toString();
      await wish.save();
    }
    req.session.wish = wish;
    req.flash("success", "Item added to wish listh");
    res.redirect(req.headers.referer);
  } catch (err) {
    console.log(err);
    res.redirect("/");
...
router.get('/wishlist', isLoggedIn, async (req, res) => {
  const user = req.user;
  try {
    const wishList = await Wish.findOne({ user: user._id });
    let products = []; // array of objects
    for (const item of wishList.items) {
      let foundProduct = (
        await Product.findById(item).populate("category")
      ).toObject();
      products.push(foundProduct);
    }
